...
CREATE OR REPLACE VIEW wp_resform_rooms_space_count AS
    SELECT
        rt.event_id,
        r.*,
        count(p.person_id) AS occupied_space_count,
        ...
        rt.space_count - count(p.person_id) AS free_space_count,
        rt.name AS room_type_name
    FROM wp_resform_rooms AS r
    LEFT JOIN wp_resform_persons AS p USING(room_id)
    LEFT JOIN wp_resform_room_types AS rt ON r.room_type_id = rt.room_type_id
    GROUP BY r.room_id;
